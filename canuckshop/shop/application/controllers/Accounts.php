...
class Accounts extends CI_Controller {
  ...
  public function forgot_password()
  {
    ...
    if ($this->form_validation->run() == TRUE)
    {
      ...
      if ($password = $this->users->select_password_by_email($email)) {
        $subject = 'Message from http://www.canuckuniforms.com';
  ...
  public function customer_profile($message = '')
  {
    ...
    $this->form_validation->set_rules('cellphone', 'Contact', 'trim|max_length[15]');
    $this->form_validation->set_rules('fax', 'Fax', 'trim|max_length[15]');

    //$this->form_validation->set_rules('email', 'Email', 'trim|required|valid_email');
    //$this->form_validation->set_rules('confirm_email', 'ConfirmEmail', 'trim|required|matches[email]');

    $this->form_validation->set_rules('password', 'Password', 'trim|min_length[4]|max_length[15]');
    ...
    $this->form_validation->set_rules('street', 'Street', 'trim|required|min_length[5]|max_length[100]');
    $this->form_validation->set_rules('city', 'City', 'trim|required|min_length[2]|max_length[50]');
    $this->form_validation->set_rules('province', 'Province', 'trim|required|min_length[2]|max_length[40]');
    $this->form_validation->set_rules('zipcode', 'ZipCode', 'trim|required|min_length[3]|max_length[40]');
