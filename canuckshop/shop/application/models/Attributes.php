...
class Attributes extends CI_Model {
  ...
  function insert_color($attname)
  {
    ...
    if ($attid) {
      ...
      .$attid.",'".$this->db->escape(ucwords(strtolower($attname)))."','"
      .$this->db->escape(ucwords(strtolower($attname)))
      ."','Colour','1')";
  ...
  {
    $update_sql="UPDATE attributes SET attname='".$this->db->escape(ucwords(strtolower($attname)))
    ."',attvalue='".$this->db->escape(ucwords(strtolower($attname)))
    ."' WHERE attid = ".$attid;
  ...
  {
    $update_sql="UPDATE attibutes SET status='".$this->db->escape($status)
    ."' WHERE attid = ".$attid;
    $query = $this->db->query($update_sql);
    if ($this->db->affected_rows() > 0) {
      return TRUE;
