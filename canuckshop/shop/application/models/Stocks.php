...
class Stocks extends CI_Model {
  ...
  function select_stocks_by_order($orderid)
  {
    ...
    $select_sql = $select_sql." ORDER BY s.stockid DESC LIMIT 20";
  ...
  function insert_stock($itemcode, $styleid, $code, $xsmall, $small, $medium, $large, $xlarge, $xxlarge, $comments)
  {
    ...
    if ($stockid) {
      ...
      .$stockid.",'".$itemcode."',".$styleid.",'".$code."',".$xsmall.",".$small.",".$medium.",".$large.",".$xlarge.",".$xxlarge.",'".$this->db->escape($comments).
      "',0,now())";
  ...
  function insert_order_item($itemcode, $styleid, $code, $orderid, $description)
  {
    $stockid = $this->next_stockid();
    if ($stockid) {
      $insert_sql="INSERT INTO stocks (stockid,itemcode,styleid,code,orderid,description,stockdate) VALUES ("
      .$stockid.",'".$itemcode."',".$styleid.",'".$code."',".$orderid.",'".$this->db->escape($description)
      ."',now())";
      //log_message('debug', $insert_sql);
      $query = $this->db->query($insert_sql);
      if ($this->db->affected_rows() > 0) {
  ...
  function update_order_numbers($stockid, $orderid, $xsmallnumbers, $smallnumbers, $mediumnumbers, $largenumbers, $xlargenumbers, $xxlargenumbers)
  {
    $update_sql="UPDATE stocks SET xsmallnumbers='".$this->db->escape($xsmallnumbers)
    ."',smallnumbers='".$this->db->escape($smallnumbers)
    ."',mediumnumbers='".$this->db->escape($mediumnumbers)
    ."',largenumbers='".$this->db->escape($largenumbers)
    ."',xlargenumbers='".$this->db->escape($xlargenumbers)
    ."',xxlargenumbers='".$this->db->escape($xxlargenumbers)
    ."' WHERE stockid = ".$stockid;
  ...
  function update_order_short($stockid, $orderid, $sidenumpos, $sidenumsize, $sidenumcolor, $sidenumtrimcolor, $sidelogopos, $sidelogoname, $sidelogocolor, $sidelogotrimcolor)
  {
  ...
  function update_order_side($stockid, $orderid, $sidenumpos, $sidelogopos)
  {
  ...
  {
    $update_sql="UPDATE stocks SET description='".$this->db->escape($description)
    ."',styleid=".$styleid
    ." WHERE stockid = ".$stockid;
