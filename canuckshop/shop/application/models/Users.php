...
class Users extends CI_Model {
  ...
  {
    $encrypted_password = $this->encrypt->encode($password);
    $update_sql="UPDATE users SET username='".$this->db->escape($username)."',password='".$encrypted_password
    ."' WHERE email = '".$email."'";
  ...
  {
    $update_sql="UPDATE users SET username='".$this->db->escape($username)
    ."' WHERE email = '".$email."'";
    $query = $this->db->query($update_sql);
    if ($this->db->affected_rows() > 0) {
      return TRUE;
  ...
  function insert($password,$orders,$username,$email,$clientid)
  {
    ...
    if ($userid) {
      ...
      .$userid.",'".$this->db->escape($encrypted_password)."',".$this->db->escape($orders)
      .",0,0,'".$this->db->escape($username)
      ."','".$this->db->escape($email)."',".$clientid
      .",1)";
