...
class Styles extends CI_Model {
  ...
  {
    $update_sql="UPDATE styles SET colorname='".$this->db->escape(ucwords(strtolower($colorname)))
  ...
  {
    $update_sql="UPDATE styles SET frontimage='".$this->db->escape($frontimage)
    ."' WHERE styleid = ".$styleid;
  ...
  {
    $update_sql="UPDATE styles SET rearimage='".$this->db->escape($rearimage)
    ."' WHERE styleid = ".$styleid;
  ...
  {
    $update_sql="UPDATE styles SET sideimage='".$this->db->escape($sideimage)
    ."' WHERE styleid = ".$styleid;
  ...
  {
    $update_sql="UPDATE styles SET status=".$this->db->escape($status)
    ." WHERE styleid = ".$styleid;
    $query = $this->db->query($update_sql);
    if ($this->db->affected_rows() > 0) {
      return TRUE;
